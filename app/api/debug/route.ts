...
import { fetchUsgsQuakes } from "@/lib/sources/usgs";
...
export async function GET() {
  ...
  const [bmkgQuakes, usgsQuakes, firmsHotspots, magmaVolcanoes, gdacsEvents, reliefWebEvents] =
    await Promise.all([
      tracked("BMKG", health, fetchAllBmkgQuakes, []),
      tracked("USGS", health, () => fetchUsgsQuakes(30, 4), []),
      mapKey
  ...
  const events: DisasterEvent[] = [
    ...bmkgQuakes.map(bmkgQuakeToEvent),
    ...wildfireClusters.map(wildfireClusterToEvent),
    ...magmaOrGvpEvents,
    ...gdacsEvents,
    ...reliefWebEvents,
  ];
  ...
  return NextResponse.json({
    ...
    sourceHealth: health,
    debug: {
      bmkgQuakeCount: bmkgQuakes.length,
      usgsQuakeCount: usgsQuakes.length,
      usgsWindowDays: 30,
